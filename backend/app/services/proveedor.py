...
def validar_nombre(nombre: str) -> str:
    ...
    if len(nombre) > 100:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El nombre no puede tener más de 100 caracteres"
        )
...
def validar_telefono(telefono: Optional[str]) -> Optional[str]:
    ...
    if not telefono_limpio.isdigit():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El teléfono solo debe contener números"
        )
    ...
    if len(telefono_limpio) > 20:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="El teléfono no puede tener más de 20 dígitos"
        )
...
async def create_proveedor(proveedor: ProveedorIn) -> ProveedorOut:
    ...
    try:
        #VALIDAR CAMPOS (antes eran @validator en el schema)
        ...
        if email_validado:
            ...
            existing_email = await db.fetch_one(query_email, values={"email": email_validado})
            if existing_email:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if telefono_validado:
            ...
            existing_telefono = await db.fetch_one(query_telefono, values={"telefono": telefono_validado})
...
async def update_proveedor(proveedor_id: int, proveedor: ProveedorIn) -> ProveedorOut:
    ...
    try:
        ...
        existing_nombre = await db.fetch_one(
            query_nombre, 
            values={"nombre": nombre_validado, "id": proveedor_id}
        )
        if existing_nombre:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
        ...
        if email_validado:
            ...
            existing_email = await db.fetch_one(
                query_email,
                values={"email": email_validado, "id": proveedor_id}
            )
            if existing_email:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
